...
const projects = [
	...
	// Smaller projects (link out to GitHub / pages)
	{
		...
		description:
			"CUDA implementations of Black‑Scholes, Barrier Options; real‑time OpenGL visualization.",
	...
	},
	{
		...
		description:
			"GUI application to find books and convert them to audiobooks using text-to-speech.",
	...
	},
	{
		...
		description:
			"Web-based document scanner with real-time border detection and shearing transformations.",
...
const ProjectsSection = () => {
	const highlight = (text: string) => {
		...
		let html = text
			.replace(
				/\$[0-9][\d\.,]*(?:\s?[kMB]|\s?million|\s?billion)?|\b[0-9][\d\.,]*k\+\b/gi,
	...
	return (
		<Section
			...
			subtitle="Selected builds with real metrics and outcomes."
			showHeader={false}
			noTopPadding
		>
				...
				{projects.map((p, idx) => (
					...
					>
						...
						>
							...
							<CardHeader>
								<CardTitle className="flex items-center gap-2 text-xl text-[hsl(var(--primary))]">
									<Rocket className="h-5 w-5" /> {p.title}
								</CardTitle>

...
const LogosMarquee: React.FC<LogosMarqueeProps> = ({ companies = defaultCompanies }) => {
  const track = [...companies, ...companies]; // duplicate for seamless scroll
  return (
    <aside aria-label="Companies I've worked with" className="py-10 md:py-12 border-t border-b bg-accent/40">
      <div className="marquee">
        <div className="marquee__track px-8 md:px-10">
          {track.map((c, i) => (
            <div key={`${c.name}-${i}`} className="flex items-center gap-4 md:gap-5 opacity-80 hover:opacity-100 transition-opacity">
              <img
                src={c.src}
                alt={`${c.name} logo`}
                ...
                loading="lazy"
              />
              <span className="text-sm md:text-base text-muted-foreground">{c.name}</span>
            </div>
          ))}
        </div>
      </div>

...
export class UIHandler {
  private static instance: UIHandler | null = null;
  ...
  private initializeJQueryComponents(): void {
    ...
    // Skills
    ($ as any)('.skill').waypoint(function () {
      $('.progress .progress-bar').each((index: number, element: HTMLElement) => {
        $(element).css("width", $(element).attr("aria-valuenow") + '%');
      });
    }, { offset: '80%' });

    // Portfolio isotope and filter
    const portfolioIsotope = ($ as any)('.portfolio-container').isotope({
      itemSelector: '.portfolio-item',
      layoutMode: 'fitRows'
    });
    
    $('#portfolio-flters li').on('click', (event: any) => {
    ...
    ($ as any)(".testimonial-carousel").owlCarousel({
      autoplay: true,
      smartSpeed: 1500,
      dots: true,
      loop: true,
   ...
   */
  private createInteractiveCursor(): void {
    if (Utils.clamp(window.innerWidth, 0, 768) === 768) return; // Skip on mobile
   ...
   * Update cursor trail
   */
  private updateCursorTrail(trail: Array<{ x: number; y: number }>): void {
   ...
   */
  private enhanceSectionCards(): void {
   ...
   */
  private enhanceNameInteraction(): void {
    const nameElement = document.querySelector('.display-3') as HTMLElement;

...
export class SceneManager {
    constructor() {
        ...
        this.raycaster = new (window.THREE || window.THREE).Raycaster();
        this.objectTrails = [];
        this.init();
    ...
    init() {
        ...
        // Camera setup
        this.camera = new THREE.PerspectiveCamera(Config.CAMERA_CONFIG.fov, window.innerWidth / window.innerHeight, Config.CAMERA_CONFIG.near, Config.CAMERA_CONFIG.far);
        ...
        // Lighting setup
        const ambientLight = new THREE.AmbientLight(Config.LIGHTING_CONFIG.ambientColor, Config.LIGHTING_CONFIG.ambientIntensity);
        ...
        this.brainPointLight = brainPointLight;
     ...
     */
    getNorthernLightsFragmentShader() {
        return `
      uniform float time;
      varying vec3 vWorldPosition;
      varying vec2 vUv;
     ...
     */
    setupRevolvingModel(model, config) {
        const userData = {
            rotationSpeed: 0.01,
            floatSpeed: 0.005,
        ...
        if (config.name === 'brain') {
            ...
            model.traverse((child) => {
                if (child.isMesh && child.material) {
                    child.material.emissive = new (window.THREE || window.THREE).Color(0x001133);
    ...
    setupThinkerProfilePicture() {
        ...
        const profileImg = document.querySelector('#thinker-profile-1');
        if (!profileImg) {
    ...
    setupCreativeBrainScrollEffects() {
        ...
        this.brainModel.traverse((child) => {
            if (child.isMesh) {
                ...
                if (child.material) {
                    child.material.emissive = new (window.THREE || window.THREE).Color(0x001122);
    ...
    handleStarInteraction(event) {
        ...
        if (this.starMaterial) {
            this.starMaterial.uniforms.mousePos.value.set(event.clientX / window.innerWidth, 1 - event.clientY / window.innerHeight);
        }
    }
    /**
     ...
     */
    createClickShootingStars(event) {
     ...
     */
    animateRevolvingModel(model, userData, scrollProgress, time) {
     ...
     */
    animateFloatingModel(model, userData, time, deltaTime) {
        if (userData?.mixer) {
     ...
     */
    animateCreativeBrain(model, userData, time) {
        const scrollProgress = Utils.getScrollProgress();
        ...
        if (this.brainPointLight) {
            this.brainPointLight.position.copy(model.position);
            this.brainPointLight.intensity = lightIntensity + Math.sin(time * 4) * 0.5;
            // Color cycle
            ...
            this.brainPointLight.color.setRGB(r, g, b);
        }
     ...
     */
    createEnhancedBrainParticleSystem(brainModel, userData) {
     ...
     */
    animateBrainParticles(particleSystem, brainModel, time, peakProgress, fadeProgress) {
        ...
        for (let i = 0; i < particleCount; i++) {
            ...
            const currentRadius = Math.sqrt(positions[i3] * positions[i3] +
